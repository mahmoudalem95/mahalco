...
(function () {
  ...
  const signals = [[146,329,'ew'],[354,171,'ew'],[171,146,'ns'],[329,354,'ns']].map(([x,y,axis])=>`<g class="signal" data-axis="${axis}" transform="translate(${x} ${y})"><rect class="signal-case" x="-6" y="-14" width="12" height="28" rx="5"/>${['red','amber','green'].map((c,i)=>`<circle data-color="${c}" class="bulb" cy="${i*8-8}" r="2.8" style="--light:${['#ff655c','#ffd66a','#62e6a1'][i]}"/>`).join('')}<g class="turn-head" transform="translate(13 0)"><rect class="signal-case" x="-5" y="-10" width="10" height="20" rx="3"/><path class="turn-arrow" data-axis="${axis}" d="M2-6H-2L0-8M-2-6L0-4M-2 5H2L0 3M2 5L0 7"/></g></g>`).join('');
  ...
  world.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500" focusable="false"><defs><radialGradient id="junction-lamp"><stop stop-color="#ffd598" stop-opacity=".55"/><stop offset="1" stop-color="#ffd598" stop-opacity="0"/></radialGradient><linearGradient id="junction-beam"><stop stop-color="#fff3d6" stop-opacity=".55"/><stop offset=".35" stop-color="#e9e9dc" stop-opacity=".22"/><stop offset="1" stop-color="#dae6e5" stop-opacity="0"/></linearGradient><filter id="junction-soft-light" x="-30%" y="-50%" width="160%" height="200%"><feGaussianBlur stdDeviation="1.6"/></filter></defs>${city}${carMarkup}${signals}${walkingMarkup}</svg>`;
  hero.prepend(world);
  ...
  const turnArrows=[...world.querySelectorAll('.turn-arrow')];
  const bulbs=[...world.querySelectorAll('.bulb')];
  // Through traffic/cyclists/pedestrians, then protected turns, with clearance between stages.
  function phase(t){
    t%=56;
    return {ew:t<9?'green':t<11?'amber':'red',ewTurn:t>=16&&t<21?'green':t>=21&&t<23?'amber':'red',
      ns:t>=28&&t<37?'green':t>=37&&t<39?'amber':'red',nsTurn:t>=44&&t<49?'green':t>=49&&t<51?'amber':'red'};
  }
  // Arc length, rather than linear interpolation, keeps turns smooth at a constant speed.
  function pose(v){
    if(v.lane===2){const p=v.pos;return ({east:[p,340,0],west:[500-p,160,180],south:[160,p,90],north:[340,500-p,-90]})[v.dir];}
    let x=v.pos,y=v.outer?305:270,heading=0;
    if(v.movement!=='straight'&&v.pos>160){
      const right=v.movement==='right',radius=right?35:110,arc=radius*Math.PI/2;
      const distance=v.pos-160,theta=Math.min(Math.PI/2,distance/radius);
      x=160+radius*Math.sin(theta);y+=(right?1:-1)*radius*(1-Math.cos(theta));heading=(right?1:-1)*theta*180/Math.PI;
      if(distance>arc)y+=(right?1:-1)*(distance-arc);
    }
    const rotation=({east:0,west:180,south:90,north:-90})[v.dir],r=rotation*Math.PI/180;
    return [250+(x-250)*Math.cos(r)-(y-250)*Math.sin(r),250+(x-250)*Math.sin(r)+(y-250)*Math.cos(r),heading+rotation];
  }
  function render(dt){
    const lights=phase(elapsed), key=Object.values(lights).join();
    if(key!==lastPhase){bulbs.forEach(b=>b.classList.toggle('on',b.dataset.color===lights[b.parentNode.dataset.axis]));turnArrows.forEach(a=>a.dataset.state=lights[a.dataset.axis+'Turn']);lastPhase=key;}
    for(const dir of ['east','west','south','north']) for(let lane=0;lane<3;lane++) {
      ...
      for(const v of group){
        ...
        if(lights[v.axis+(v.movement==='straight'?'':'Turn')]!=='green'&&v.pos<=stop) limit=Math.min(limit,stop);
    ...
    vehicles.forEach((v,i)=>{
      const coords=pose(v);
      nodes[i].dataset.movement=v.movement;
      nodes[i].dataset.approach=v.dir;
    ...
    walkers.forEach((person,i)=>{
      const local=(elapsed-(person.axis==='ns'?28:0)+56)%56;
      const cycle=Math.floor((elapsed-(person.axis==='ns'?28:0))/56);
